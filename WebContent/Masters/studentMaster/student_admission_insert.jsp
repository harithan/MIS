...
String newpath="D:\\misfinal\\misFinal\\WebContent\\images\\"+rollno+".jpg";
String newpath1="../../images/"+rollno+".jpg";
String file1=request.getParameter("file1");
File imgfile = new File(file1);
...
int flag=0;
  ...
  {   
	...
	pst=con.prepareStatement("update student_tab set admn_num=?,student_name=? ,branch_name=?, sem=?,photo=?,path=? where roll_num=?");
	pst.setInt(1,admnno);
	pst.setString(2,fullname);
	pst.setString(3,branch);
	pst.setInt(4,sem);
	
	pst.setBinaryStream(5,fin,(int)imgfile.length());
	pst.setString(6,newpath1);
...
<%
  ...
  if(flag!=1)
  {
	...
	pst=con.prepareStatement("insert into student_tab(admn_num,roll_num,student_name,branch_name,sem,photo,path) values(?,?,?,?,?,?,?)");
	pst.setInt(1,admnno);
